...
| `taskq.store` | Own `tasks.json`, task CRUD, atomic JSON write/read, thread lock, cross-process lock adapter, corruption detection, fault outcomes, and schema migration primitives shared by persistence owners. Also owns `clear_all()` — a single cross-process-excluded call that resets `tasks.json`, `breaker.json`, and `cache.json` in one critical section. | `taskq.config`, `taskq.models`. | No subprocess, retry, breaker policy, or CLI formatting. |
...
| `taskq.breaker` | Own global breaker state, threshold counting, cooldown/half-open admission, and `breaker.json` persistence through store primitives; expose `reset_persisted()` (called only from `store.clear_all()`) to reset the file to `{version:1, state:CLOSED, failure_count:0, opened_at:null}`. | `taskq.config`, `taskq.models`, `taskq.store`. | No subprocess invocation, retry scheduling, or solo file rewriting. |
| `taskq.cache` | Own SHA-256 signatures, TTL decisions, and `cache.json` persistence through store primitives; expose `clear_all()` (called only from `store.clear_all()`) to reset the file to `{version:1, entries:{}}`. | `taskq.config`, `taskq.models`, `taskq.store`. | No subprocess execution, breaker transitions, or solo file rewriting. |
| `taskq.cli` | Own argparse grammar, command validation orchestration, output rendering, `--inject-fault=<scenario>` parsing/test-monkeypatch boundary (see §2.3.1), coordinated `clear` subcommand dispatch via `store.clear_all()`, and exception-to-exit-code mapping. | `taskq.config`, `taskq.models`, `taskq.store`, `taskq.executor`, `taskq.breaker`, `taskq.cache`. | No direct `subprocess.run` or hand-built persistence protocol. |
...
1. **CLI parser boundary** — `taskq.cli` registers `--inject-fault=<scenario>` as an argparse top-level optional, parsed **before** the subcommand layer. When parsed in a production invocation, `cli` exits with code 2 and stderr `inject-fault rejected in production`; no fault code runs.
2. **Service boundary** — the parsed scenario value flows through a single `taskq.fault` module attribute (a string sentinel or `None`). `taskq.store`, `taskq.breaker`, and `taskq.cache` consult that attribute on the relevant write path; when it is `None` the fault hooks are no-ops.
3. **Test-only entry** — unit/integration tests either invoke the CLI with `--inject-fault=<scenario>` (the integration path), or monkeypatch `taskq.fault.scenario` directly (the unit path). Both paths share the same downstream hooks, so production cannot enable a fault scenario without passing through the CLI parser's production-reject branch.

This three-stage boundary satisfies the canonical SPEC.md §5.3 contract that fault injection is "CLI flag or test monkeypatch" and is forbidden on the production code path.
...
|---|---|---|
| `cli` → `store` | `add(task)`, `get(id)`, `update(id, fields)`, `iter_tasks(status)`, `clear_all()` | Task record or persistence result; explicit corruption/migration errors. `clear_all()` empties `tasks.json`, `breaker.json`, and `cache.json` in one cross-process-excluded critical section (it calls `breaker.reset_persisted()` and `cache.clear_all()` under the same exclusive lock acquired by `store`). |
...
security_design:
  ...
  threats:              # STRIDE-lite — every boundary has at least one threat
    ...
    - id: T-07
      ...
      mitigation: "cli accepts --inject-fault only in the explicitly enabled test/development path and rejects it in production"
      owner_module: "taskq.cli"
      nfr: NFR-07
